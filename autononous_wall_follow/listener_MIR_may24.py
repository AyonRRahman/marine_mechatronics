...
kp = [-0.0005,-0.01,0]           # surge yaw depth
kd = [-0,0, 0.0]        # surge yaw depth
ki = [-0.00,0.0,0]          # surge yaw depth
...
#     global mod 
#     global flag_mode_stabilize
#     global wait_after_explore

#     new_angle = (( angle_yaw_a0 +math.pi/2 + 3.0*math.pi)%(2.0*math.pi) - math.pi) 
    
#     if np.abs(current_state[1] - angle_yaw_a0)<=(3.14/2 + 0.05):
#         exploring_heading=False
#         flag_mode_stabilize = False
#         print(f"angle_yaw_a0 {angle_yaw_a0}")

#         angle_yaw_a0 = new_angle
#         rospy.loginfo('Done exploring')
#         print(f"angle_yaw_a0 {angle_yaw_a0}")
#         print(current_state[1])
#         print(current_state[0])
#         wait_after_explore = True
#         # time.sleep(0.1)
#         return
...
#                             kp=kp,ki=ki,kd=kd,
#                             state=current_state,
#                             destination =[current_state[0],new_angle ,current_state[2]], 
#                             floatability=floatability
#                             )
#     counter_explore_heading+=1
#     # print(f'explore angle {angle_yaw_a0+mod*counter_explore_heading}')
#     # print(current_state[0])
#     # if np.abs(mod*counter_explore_heading)>=70*3.14159/180:
#     #     mod *= -1
...
def write_thurster_values(controller_signal, stop_heading=False):
    global control_mode
    ...
    if sum(control_mode)>0:
        ...
        # print(f"control signal in pwm: {Correction_surge}")

    if stop_heading:
        Correction_yaw=1500
    # Correction_yaw=1500
    # Correction_surge = 1437
    if (Correction_surge)>1541:
        Correction_surge=1541
    setOverrideRCIN(1500, 1500, Correction_depth, Correction_yaw, Correction_surge, 1500)
    
global prev_pinger_distance
prev_pinger_distance = 100
...
global turn_left
turn_left = False
global count_stable_heading
count_stable_heading = 0
...
def pingerCallback(data):
    ...
    if go_forward:
        ...
        autonomous_turn_left(angle = 90)
    
def autonomous_turn_left(angle = 90):
    global go_forward
    global turn_left
    ...
    if pinger_confidence<50:
        #bad confidence stop thrusters
    ...
    if (np.abs(imu_data['yaw'] - (angle_yaw_a0 + angle))<0.02):
        #increase count as error is in 2 degree
        count_stable_heading+=1

    elif count_stable_heading!=0:
        #reset the count as it was false data
        print('deleting false data')
        count_stable_heading = 0
    

    if count_stable_heading>=3:
        #heading is stable so go forward now
        print('found stable heading after rotation. now going forward')
        go_forward = True
        turn_left = False
        angle_yaw_a0 += angle
        setOverrideRCIN(1500, 1500, 1500, 1500, 1500, 1500)
    ...
    if sum(control_mode)>0 and set_mode[2] and arming:
            controller_signal = PID_control(
                            kp=kp,ki=ki,kd=kd,
                            state=current_state,
                            destination =[current_state[0],angle_yaw_a0 + angle ,current_state[2]], 
                            floatability=floatability
                            )

            write_thurster_values(controller_signal, stop_heading=False)
...
def autonomous_control_wall_follow():
    ...
    if pinger_confidence>=50:
        print(f'good confidence {pinger_confidence}')
        current_state = [pinger_distance, imu_data['yaw'], depth_wrt_startup]
        prev_pinger_distance=pinger_distance
        
        
    else:
        print(f'bad confidence {pinger_confidence}')
        setOverrideRCIN(1500, 1500, 1500, 1500, 1500, 1500)
        current_state = [prev_pinger_distance, imu_data['yaw'], depth_wrt_startup]
        return
        
        
    if current_state[0]<=900:
        rospy.loginfo('distance less than 90. Now exploring')
        # setOverrideRCIN(1500, 1500, 1500, 1500, 1500, 1500)
        # return
        ...
        if sum(control_mode)>0 and set_mode[2] and arming:
            controller_signal = PID_control(
                            kp=kp,ki=ki,kd=kd,
                            state=current_state,
                            destination =[900,angle_yaw_a0 ,current_state[2]], 
                            floatability=floatability
                            )
